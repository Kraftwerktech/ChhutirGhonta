...
function SubstituteScheduling() {
  ...
  return (
    <div className="px-8 pb-8 mt-1">
      <h3 className="text-lg font-semibold ml-4 mb-2">Substitute Scheduling</h3>

      <div className="w-full bg-white pb-0 shadow-sm rounded-lg mb-0">
        <div className="flex flex-wrap gap-4 md:gap-6 p-4">
          {/* Date Select */}
          <div className="relative w-full md:w-[152px]">
            ...
            />
            <CiCalendar className="absolute w-6 h-6 text-[#BB5042] right-3  top-1/2 transform -translate-y-1/2 pointer-events-none" />
      ...
      <div className='ml-4'>
        <span className='text-[#151515] text-[16px]'>3(8) Teacher’s are available</span>
      </div>

      {/* Student Table */}
      <div className="w-full bg-white p-0 shadow-sm rounded-lg">
        ...
        {displayedStudents.map((student, index) => (
          <tr key={student.id} className="border-b h-[92px] text-sm text-gray-700 text-center">
            ...
            <td className="px-4 py-2">
              <button className="w-[76.67px] h-[60px] bg-[#F8EEEC] flex items-center justify-center">
                {student.six}
                <RxCrossCircled className="text-red-400 h-6 w-6" />
              </button>
            </td>
            <td className="px-4 py-2">
              <button className="border-[1px] border-[#BB5042] text-[#BB5042] w-[103px] h-[44px] rounded-md">
                Assign
              </button>
            </td>
          </tr>
